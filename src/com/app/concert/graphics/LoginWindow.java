...
/**
 * An object representing a login window to collect a user's login data
 * 
...
public class LoginWindow extends BaseWindow {
	...
	public LoginWindow() {
		...
		// Creates the hidden version of the password's text field and adds it to the
		// panel
		...
		LoginWindow thisWindow = this; // Creates a pointer for this login window to be used in the following
										// ActionListener
		btnCancel.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				thisWindow.dispatchEvent(new WindowEvent(thisWindow, WindowEvent.WINDOW_CLOSING)); // Closes the login
																									// window
			}
		});
		...
		btnSubmit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (new UserHandler().searchForUser(txtEmail.getText(), password)) {

					int type = Email.getEmailType(txtEmail.getText());

					switch (type) {

					case 1:
						HostMainPage hmp = new HostMainPage(self);
						hmp.setDefaultCloseOperation(DISPOSE_ON_CLOSE);
						hmp.setVisible(true);
						break;
					case 2:
						new ArtistEditDescription(self, new User(uh.getID2(txtEmail.getText()), 
								uh.getName(txtEmail.getText()), txtEmail.getText(), password )).setVisible(true);;
						break;
					case 0:
					default:
							new UserMainPage2(self, new User(uh.getID2(txtEmail.getText()), 
									uh.getName(txtEmail.getText()), txtEmail.getText(), password ));
					}

				} else {
					JOptionPane.showMessageDialog(self, "User not found.");
				}
			}
		});
		...
		Image image = new ImageIcon(this.getClass().getResource("/concertdb/login_image.png")).getImage();
